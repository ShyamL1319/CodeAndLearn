...
//        System.out.println(Math.abs(9-101));
    ...
    public int reversPair(int nums[]) {
        int res = mergeSortR(nums, 0, nums.length - 1);
        return res;
    }

    public int mergeSortR(int nums[], int low, int high) {
        if (low >= high)
            return 0;
        int mid = (low + high) / 2;
        int cnt = mergeSortR(nums, low, mid);
        cnt += mergeSortR(nums, mid + 1, high);
        cnt += mergeR(nums, low, mid, high);
        return cnt;

    }

    public int mergeR(int nums[], int low, int mid, int high) {
        int cnt = 0;
        int size = high - low + 1;
        int temp[] = new int[size];
        int j = mid + 1;
